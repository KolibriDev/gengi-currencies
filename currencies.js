...
export const formatCurrencyName = (providedName) => {
  // Change 'dalur, bandarískur' to 'bandarískur dalur'
  const parts = providedName.split(',');
  let value =
    parts.length > 1 ? `${parts[1].trim()} ${parts[0].trim()}` : parts[0];
...
export function a(parsedXml) {
  const currencies = {};
  parsedXml.Rate.sort((a, b) =>
    a.CurrencyCode[0].localeCompare(b.CurrencyCode[0])
  ).forEach((currency) => {
